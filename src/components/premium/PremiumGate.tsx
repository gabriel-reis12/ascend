...
export function PremiumGate({ children, title, description }: PremiumGateProps) {
  ...
  const tx = (value?: string) => isEnglish && value ? translateUiText(value) : value;
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      if (functionError) {
        ...
        throw new Error(errBody?.error || functionError.message || l("Erro ao conectar ao servidor de checkout.", "Could not connect to the checkout server."));
      }
      ...
      if (data?.url) {
        ...
        throw new Error(l("URL do Stripe Checkout não foi gerada pelo servidor.", "Stripe Checkout URL was not generated by the server."));
      }
    } catch (err: any) {
      console.error('[PremiumGate] Erro ao redirecionar para checkout:', err);
      setError(tx(err.message) || l('Falha ao iniciar o portal de pagamento. Tente novamente mais tarde.', 'Failed to start the payment portal. Try again later.'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-purple-500/25 bg-[#0A0A0E] p-8 shadow-[0_0_40px_rgba(147,51,234,0.08)] text-center max-w-2xl mx-auto my-6">
      ...
      <h2 className="text-xl sm:text-2xl font-black uppercase tracking-wider text-white font-orbitron italic">
        {tx(title) || l("Fenda Dimensional Restrita", "Restricted Dimensional Gate")}
      </h2>
      <p className="mt-1 text-[10px] font-black uppercase tracking-[0.2em] text-purple-400">
        {l('Requer Chave do Despertar (Nível Premium)', 'Requires Awakening Key (Premium Level)')}
      </p>

      {/* Descrição */}
      <p className="mt-4 text-sm leading-relaxed text-gray-400 max-w-md mx-auto">
        {tx(description) || l('Esta área contém recursos avançados do Sistema. Desperte seu potencial completo e quebre os limites do seu treino.', 'This area contains advanced System features. Awaken your full potential and break your training limits.')}
      </p>
      ...
      <div className="mt-6 p-4 rounded-2xl border border-white/5 bg-black/40 text-left max-w-md mx-auto space-y-3">
        <h4 className="text-[10px] font-black uppercase tracking-widest text-purple-400 font-orbitron">
          {l('Benefícios do Despertar Premium', 'Premium Awakening Benefits')}
        </h4>
        <ul className="space-y-2 text-xs font-semibold text-gray-300">
          ...
          <li className="flex items-center gap-2">
            <CheckCircle2 size={13} className="text-emerald-400 shrink-0" />
            <span>{l('Histórico de Treinos e Gráficos de Evolução Física', 'Workout history and physical progression charts')}</span>
          </li>
          <li className="flex items-center gap-2">
            <CheckCircle2 size={13} className="text-emerald-400 shrink-0" />
            <span>{l('Acesso às Raids Finais: Últimos 3 Bosses', 'Access to Final Raids: Last 3 Bosses')}</span>
          </li>
          <li className="flex items-center gap-2">
            <CheckCircle2 size={13} className="text-emerald-400 shrink-0" />
            <span>{l('Módulo Fortuna de Gestão Financeira & Metas', 'Fortune module for financial management & goals')}</span>
          </li>
        </ul>
      </div>
      ...
      <div className="mt-8 space-y-4 max-w-md mx-auto">
        ...
        >
          {loading ? (
            <>
              ...
              {l('Sincronizando com Stripe...', 'Synchronizing with Stripe...')}
            </>
          ) : (
            <>
        ...
        {error && (
          ...
          >
            ...
            <div>
              <span className="font-bold">{l('ANOMALIA NO CHECKOUT:', 'CHECKOUT ANOMALY:')}</span> {error}
              <p className="mt-1 text-gray-500 font-medium">{l('Nota: Certifique-se de que a variável STRIPE_SECRET_KEY e as chaves de teste estão configuradas no Supabase.', 'Note: Make sure STRIPE_SECRET_KEY and the test keys are configured in Supabase.')}</p>
            </div>
          </motion.div>
        )}
      </div>
