...
- **Implementação do Sistema de Onboarding Tour (Tutorial Interativo)**:
  ...
  - **Solução Aplicada**:
    ...
    5. **Mapeamento de Âncoras do Sistema (`Dashboard.tsx`)**: Injetados identificadores no DOM para mapear os 5 pilares fundamentais da interface principal: HUD do Caçador (`tour-hud`), Atalhos Rápidos (`tour-shortcuts`), Mural de Missões (`tour-quests`), Janela de Atributos RPG (`tour-status`) e Painel Biométrico (`tour-biometrics`).
    6. **Painel de Recalibração de Guias (`Settings.tsx`)**: Adicionado o painel "Diretrizes de Sistema" na tela de Ajustes contendo um botão de toque único roxo cyberpunk para limpar a chave do tutorial do `localStorage` e re-invocar o protocolo de treinamento interativo instantaneamente a qualquer momento.
